...
class BVH(object):
    ...
    def hit(self, ray_origin, ray_direction, vertices, triangles, bounding_boxes, bvh):
        if bvh.ntris > 0:
            return hit_triangles(ray_origin, ray_direction, vertices, \
                                 triangles[bvh.index:bvh.index+bvh.ntris])
        ...
        for child in bvh.childs:
            if BoundingBox(bounding_boxes[child.aa], bounding_boxes[child.bb]).hit(ray_origin, ray_direction):
                triangle_candidate, distance_candidate = self.hit(ray_origin, ray_direction, \
